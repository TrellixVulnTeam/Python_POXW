...
"""
  ...
  $Id: //eng/vdo-releases/magnesium-rhel7.5/src/python/vdo/vdomgmnt/VDOService.py#2 $
...
class VDOService(Service):
    ...
    def status(self):
        ...
        if os.getuid() == 0:
            ...
            try:
                status[self.vdoStatisticsKey] = self.vdostats.get(self.getPath(), _("not available"))
    ...
    def __init__(self, name, conf, **kw):
        ...
        self.cpuThreads = self._defaultIfNone(kw, 'vdoCpuThreads',
                                              Defaults.cpuThreads)
        # logicalBlockSize default 4K
        self.logicalBlockSize = 4096
    ...
    @property
    def vdostats(self):
        if VDOService.vdostats_cache:
            return VDOService.vdostats_cache

        try:
            result = runCommand(['vdostats', '--verbose'])
            VDOService.vdostats_cache = yaml.load(result)
        except Exception as e:
            VDOService.vdostats_cache = dict()
