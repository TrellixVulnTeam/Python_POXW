...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Nmap find ip not use.")
    parser.add_argument('ip', help="Input the ip that need scan, like 10.10.100.0/24")
    args = parser.parse_args()

    ip = args.ip

    scaner = PortScanner()
    res = scaner.scan(hosts=ip, arguments="-sP")


    import json
    print(json.dumps(res, indent=4))
    for ip in sorted(list(res.get("scan", dict()).keys())):
        print(ip)
